...
class PatchCoreNoisingTrainer():
	def __init__(self, cfg):
		...
		if cfg.dist:
			if cfg.trainer.scaler in ['none', 'native']:
				...
				raise ValueError('Invalid scaler mode: {}'.format(cfg.trainer.scaler))
		# =========> dataset <=================================
	...
	def forward(self):
		"""Forward pass through the network (RD style)."""
	...
	def backward_term(self):
		"""Compute cosine similarity loss (RD style)."""
	...
	def optimize_parameters(self):
		...
		if self.loss_scaler is not None:
			...
			if self.cfg.trainer.clip_grad:
				self.loss_scaler.unscale_(self.optim)
				dispatch_clip_grad(
					self.net.parameters(),
					value=self.cfg.trainer.clip_grad,
					mode=self.cfg.trainer.clip_mode
				)
			self.loss_scaler.step(self.optim)
	...
	def test(self):
		...
		while batch_idx < test_length:
			...
			scores, maps = self.net.module.compute_anomaly_score(self.imgs) if self.cfg.dist else self.net.compute_anomaly_score(self.imgs)

			# scores and maps are already numpy arrays from model
			...
			if self.cfg.vis:
				...
				vis_rgb_gt_amp(self.img_path, self.imgs, self.imgs_mask.cpu().numpy().astype(int), maps, self.cfg.model.name, root_out, self.cfg.data.root.split('/')[1])

			imgs_masks.append(self.imgs_mask.cpu().numpy().astype(int))
			anomaly_maps.append(maps)
	...
	def save_checkpoint(self):
		if self.master:
			checkpoint_info = {'net': trans_state_dict(self.net.state_dict(), dist=self.cfg.dist),
			...
			if hasattr(self.cfg, 'wandb') and self.cfg.wandb.enable and self.cfg.wandb.log_model:
				# Get average AUROC as metric for best model
				if len(self.cls_names) > 1:
					auroc_avg = self.metric_recorder.get('mAUROC_sp_max_Avg', [0])[-1] if self.metric_recorder.get('mAUROC_sp_max_Avg') else 0
				else:
					auroc_avg = self.metric_recorder.get(f'mAUROC_sp_max_{self.cls_names[0]}', [0])[-1] if self.metric_recorder.get(f'mAUROC_sp_max_{self.cls_names[0]}') else 0
