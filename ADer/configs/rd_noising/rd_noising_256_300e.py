"""
RD Noising configuration for MVTec dataset - 300 epochs with noise warmup.

Two-phase training:
Phase 1: Build memory bank from teacher features
Phase 2: Train decoder with adaptive noise (with warmup)
"""
...
class cfg(cfg_common, cfg_dataset_default, cfg_model_rd_noising):

    def __init__(self):
        cfg_common.__init__(self)
        cfg_dataset_default.__init__(self)
        cfg_model_rd_noising.__init__(self)
        ...
        self.model.kwargs = dict(
            ...
            noise_layers=[0, 1, 2],
            enable_noise=True,
            # Batch size for distance computation (avoid OOM)
        ...
        # ==> trainer
        self.trainer.name = 'RDNoisingTrainer'
